...
Following the approach in \cite{Patra2018b, Bevilacqua2019}, we define $\left(M(A), P_{M(A)}\right)$, where $A$ is a set of assumptions, $M(A)$ is the model composed by those assumptions, and $P_M$ is a probability measure in the input space of $M$. The measure $P_M$ can become a Dirac delta after the solution of an inverse problem for the optimal reconstruction of a particular flow, but this would reduce the predictive capabilities of the model, where we have to investigate the outcomes over a nontrivial input space.
...
The class of assumptions that we specifically test in this study are the assumptions on the rheology of the flows. In particular they model the different dissipation mechanisms embedded in $S_x, S_y$, and that cause an abundance of models with much controversy on the most suitable model. %We focus here on three models composed by different assumptions for essentially the same class of flows.

\subsection{Overview of the models}\label{subsec:Models}
We will define our approach and illustrate it using three models for large scale mass flows incorporated in our large scale mass flow simulation framework  TITAN2D \citep{Patra2005,Patra2006, Yu2009, Aghakhani2016}. The following section is summarized from \cite{Bevilacqua2019} and reported in this study for the sake of clarity.
