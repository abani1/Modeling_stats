...
In our case study, $(F_1,F_2):=($\textbf{LHS}$_1,$\textbf{LHS}$_2)$, i.e. the left-hand side terms in the momentum equation, while $(F_3,\dots,F_6):=($\textbf{RHS}$_1,\dots,$\textbf{RHS}$_4)$ are the right-hand side terms. These forces can take different formulations in the different models, which are detailed in section \ref{sec:QoIs}. This type of procedure can be applied to any additive decomposition of the physical forces. The analysis of $RHS$ can inform on local effects, while $LHS$ concerns the of un-local dynamics in space and time.

In general we are projecting the forces on the slope direction, or along the velocity field obtaining the power related to the force. Hence, in the following the forces are scalar and not vectorial terms. It is important to remark that all the forces are hidden quantities (see above), depending on the input variables, and they are thus considered as random variables. The degree of contribution of those force terms is significantly variable in space and time, and we define the \emph{contribution coefficients} to model this. The definitions are not depending on the location, but all the results will significantly depend of that choice. Next notation will assume to be in a selected location $x=L_k$, where $k\ge 1$. In the following we will focus on \textbf{RHS} forces, although some testing including \textbf{LHS} forces has also been accomplished.
...
In sections \ref{stat1} and \ref{stat2} we display the contribution coefficients as a function of time, assuming $\Phi=\Phi_1$. Moreover, we report the dominance factors plots, another very useful tool to read the dynamical significance of the forces as a function of time and location.
...
We can find several instances of parameter choices in literature, and in general the choice depends on the case study. In all the models it is defined a basal friction stress (see $RHS_2$ in in \ref{sec:QoIs}). However, PF interpolates different effective friction angles, while VS includes a velocity based additional term. We tested that a direct correlation of effective friction angles is problematic to do. Hence, we did not follow such approach. In contrast, we make a preliminary testing on the extreme values of the parameter space, i.e. giving max volume -- min resistance, and min volume -- max resistance. 

\paragraph{General definitions}
We assume what follows to simplify testing. 
\begin{itemize}
\item In MC, $\phi_{int}-\phi_{bed}=\Delta \phi \in [2^{\mathrm{\circ}}, 10^{\mathrm{\circ}}]$ \citep{Dalbey2008}.

\item In PF, $\phi_2-\phi_1=\Delta \phi_{12} \in [10^{\mathrm{\circ}}, 15^{\mathrm{\circ}}]$. Moreover, $\beta \in [0.1, 0.85]$, $\phi_3=\phi_1+1^\mathrm{\circ}$, and $\mathcal{L}$ is equal to 1 dm and 1 mm in the two case studies, respectively \citep{PouliquenForterre2002,ForterrePouliquen2003,Gray2014, Barker2015},.

\item In VS, $\xi$ uniform sampling is accomplished in log-scale. Values of $\xi$ between 250 and 4,000 $m/s^2$ have been described for snow avalanches \citep{Salm1993,Bartelt1999,Gruber2007}.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Inclined plane setup. Contours of $h = 1.0 \ mm$ at last simulated snapshot ($t = 1.5$ sec) for simulated flows with \underline{\texttt{minimum runout}} obtained from \underline{\emph{\textbf{min volume -- max resistance}}}, and \underline{\texttt{maximum runout}} obtained from \underline{\emph{\textbf{max volume -- min resistance}}}. {\color{blue} \textbf{---}} : MC, {\color{red} \textbf{---}} : PF, \textbf{---} : VS.}\label{fig:Ramp-MaxMinRunouts}
\end{figure}
...
\subsubsection*{Flow Height - $h$ local measurements}
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{NewTest/Height.png}
        \caption{Records of flow height, $h(L,t)$, at four spatial locations of interest. Bold line is mean value, dashed/dotted lines are 5$^{th}$ and 95$^{th}$ percentile bounds. Different rheology models are displayed with different colors.}
        \label{fig:Ramp-LM-H}
\end{figure}

\subsubsection*{Flow Lateral Extent - $E(t)$, (not up to date)}
\begin{figure}[H]
        \centering
...
\subsubsection*{Flow Area - $A(t)$, (not up to date)}
\begin{figure}[H]
        \centering
...
\subsubsection*{Flow Speed - $\Vert \underline{\mathbf{u}} \Vert$, local measurements}
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{NewTest/Velocity.png}
        \caption{Records of flow speed, $\Vert \underline{\mathbf{u}} \Vert(L,t)$, at four spatial locations of interest. Bold line is mean value, dashed/dotted lines are 5$^{th}$ and 95$^{th}$ percentile bounds. Different rheology models are displayed with different colors.}
        \label{fig:Ramp-LM-Vel}
\end{figure}

\subsubsection*{Flow Speed - $\Vert \underline{\mathbf{u}} \Vert$, spatial average, (not up to date)}
\begin{figure}[H]
        \centering
...
\subsubsection*{Froude Number - $\frac{\Vert \underline{\mathbf{u}} \Vert}{\sqrt{gh}}$, local measurements}

\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{NewTest/Froude.png}
        \caption{Records of Froude number at four spatial locations of interest. Bold line is mean value, dashed/dotted lines are 5$^{th}$ and 95$^{th}$ percentile bounds. Different rheology models are displayed with different colors.}
        \label{fig:Ramp-LM-Fr}
\end{figure}

\subsubsection*{Froude Number - $\frac{\Vert \underline{\mathbf{u}} \Vert}{\sqrt{gh}}$, spatial average (not up to date)}
\begin{figure}[H]
        \centering
...
\subsubsection*{Flow Acceleration - $\Vert \underline{\mathbf{a}} \Vert$, local measurements}
...
\newpage
\subsection{Forces and Powers - (not up to date)}
\subsubsection*{Force LHS$_1$ - spatial average}
...
\newpage
\subsubsection*{Force LHS$_2$ - spatial average}
...
\newpage
\subsubsection*{Force RHS$_1$ - spatial average}
...
\newpage
\subsubsection*{Force RHS$_2$ - spatial average}
...
\newpage
\subsubsection*{Force RHS$_3$ - spatial average}
...
\newpage
\subsubsection*{Force RHS$_4$ - spatial average}
...
\subsection{Contribution coefficients - RHS terms}
\begin{figure}[H]
         \centering
...
\subsection{Dominance factors - RHS terms}\label{stat1}
\begin{figure}[H]
         \centering
...
\subsubsection*{Flow Height - $h$ local measurements}
\textbf{Mohr-Coulomb rheology.}
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{MC&VS_51/Height_MC.png}
        \caption{Records of average flow height, $h(L,t)$, at 51 spatial locations of interest, according to MC model.}
        \label{fig:Ramp-LM-H}
\end{figure}

\textbf{Voellmy-Salm rheology.}
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{MC&VS_51/Height_VS.png}
        \caption{Records of average flow height, $h(L,t)$, at 51 spatial locations of interest, according to VS model.}
        \label{fig:Ramp-LM-H}
\end{figure}
...
\begin{figure}[H]
        ...
        \caption{Records of average flow speed, $\vert\underline{u}\vert(L,t)$, at 51 spatial locations of interest, according to MC model.}
        \label{fig:Ramp-LM-H}
\end{figure}
...
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{MC&VS_51/Velocity_VS.png}
        \caption{Records of average flow speed, $\vert\underline{u}\vert(L,t)$, at 51 spatial locations of interest, according to VS model.}
        \label{fig:Ramp-LM-H}
\end{figure}
...
\begin{figure}[H]
        ...
        \caption{Records of average Froude Number at 51 spatial locations of interest, according to MC model.}
        \label{fig:Ramp-LM-H}
\end{figure}
...
\begin{figure}[H]
         \centering
        \includegraphics[width=1\textwidth]{MC&VS_51/Froude_VS.png}
        \caption{Records of average Froude Number at 51 spatial locations of interest, according to VS model.}
        \label{fig:Ramp-LM-H}
\end{figure}


\subsection{Dominance factors - RHS terms}\label{stat2}
