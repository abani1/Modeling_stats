...
OA are very useful for defining Latin Hypercubes which are also forced to fill the space (or its r-dimensional subspaces, for a chosen $r<d$) in a more robust way, at the cost of potentially requiring a larger number of points than in a traditional Latin Hypercube sampling. These special experimental designs share the space filling properties of a regular grid and the statistical stratification of a Latin Hypercube, at the cost of an increased number of required points. In particular, inside each r-dimensional projection, the OA-based design fills the space like a regular grid at a coarse scale, but it is still an Latin Hypercube Sampling at a fine scale. A complete proof can be found in \citep{Tang1993} and it is a straightforward verification of the required properties.
