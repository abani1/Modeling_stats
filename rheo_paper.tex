...
\begin{figure}[H]
    \includegraphics[width=0.80\textwidth]{InclinedPlane/inclPlaneFig.jpg}
    \centering
...
The first case study assumes very simple boundary conditions, and corresponds to an experiment fully described in \cite{Webb2004, Bursik2005, WebbBursik2016}. It is a classical flow down an inclined plane set-up, including a change in slope to an horizontal plane (Fig. \ref{fig:Ramp-first}).
...
\paragraph{Simulated quantities} The simulation algorithms can be classified as:
$$\textmd{(1)INPUT VARIABLES} \longrightarrow \textmd{(2)HIDDEN QUANTITIES} \longrightarrow \textmd{(3)OBSERVABLE OUTPUTS}$$
The \emph{input variables} are the parameters of $M$, i.e. volume, rheology coefficients, but include also the initiation site and geometry, and the digital elevation map. The \emph{hidden quantities} include the stress terms in the Newton Equations that rule the simulation, their powers, other dynamical quantities as $Fr$. Those are hidden to the observation, but they directly depend on the parameters, and represent a fundamental link between the parameters and the observable outputs. Moreover, the models share some of those terms while change others, and this enables a detailed comparison of the real physics below the curtain. Finally, the \emph{observable outputs} include what can be measured in space and time: e.g. flow height, lateral extent, area, velocity and acceleration. In the sequel (2) and (3) are also called quantities of interest (QoI).
...
Our sampling technique of the input variables is based on the Latin Hypercube Sampling (LHS) idea, and in particular, on the improved space-filling properties of the orthogonal array-based Latin Hypercubes (see Appendix \ref{Appendix}). The LHS is performed over $[0,1]^3$ for the MC and VS input parameters, and $[0,1]^4$ for PF input parameters. Those adimensional samples are homothetically transformed to fill the required intervals.

\subsection{Definition and consistency of the Input parameters}\label{consistency}
...
\par\noindent $\mathcal{L}$ is fixed and depends on the granular scale, $\phi_3=\phi_1+1^\mathrm{\circ}$ \citep{PouliquenForterre2002} due to their reduced sensitivity.
...
We relied on the published parameter choices to decide a comprehensive parameter range.
\begin{itemize}

\item \textbf{Material Volume:}
\begin{table}[H]
\caption{Material volume characteristics used for simulations of \\ flow down the inclined plane setup.}
	\begin{center}
	\label{tab:Ramp-Volume}
		\begin{tabular}{l|c|c}
                \hline \hline
                & \textbf{\small Uncertainty Range} & \textbf{\small Mean Value} \\
                \hline
                \textbf{\small Material Volume} & $[449.0 \ ,\ 607.0] \ (cm^3)$ &  $528.0 \ (cm^3)$\\
                                 & {\scriptsize ($\pm15\%$ of mean value)} & \\
                \hline \hline
		\end{tabular}
	\end{center}
\end{table}
...
\vskip.3cm\noindent \textbf{MC} - $\phi_{bed} \in [15^{\mathrm{\circ}}, 30^{\mathrm{\circ}}]$, $\quad \Delta\phi \in [2^{\mathrm{\circ}}, 10^{\mathrm{\circ}}]$.

\vskip.3cm\noindent \textbf{PF} - $\phi_2 \in [15^{\mathrm{\circ}}, 35^{\mathrm{\circ}}]$, $\quad\Delta\phi_{12} \in [5^{\mathrm{\circ}}, 15^{\mathrm{\circ}}]$, $\quad\beta \in [0.1, 0.85]$, $\quad \mathcal{L}=10^{-3} \ (m)$, $\quad \phi_3=\phi_1 + 1^{\mathrm{\circ}}$.
...
In this more complex large scale simulation we couldn't rely on previous studies to decide the parameter range of Pouliquen-Forterre and Voellmy-Salm rheologies. Hence we made a preliminary testing with on the extremal cases of the different models, i.e. max volume -- min resistance, and min volume -- max resistance. We tentatively coupled the runout observed to the Mohr-Coulomb case, obtanining the following ranges. Figures \ref{Colima-MaxMinExtentsMC}, \ref{Colima-MaxMinExtentsPF}, \ref{Colima-MaxMinExtentsVS} display the map of max flow height and max velocities observed.

\begin{itemize}

\item \textbf{Material Volume:}
\begin{table}[H]
\caption{Material volume characteristics used for Block and Ash flow \\ simulations at Volc{\'a}n de Colima.}
\label{tab:Colima-Volume}
\begin{center}
\begin{tabular}{l|c|c}
                \hline \hline
                & \textbf{\small Uncertainty Range} & \textbf{\small Mean Value} \\
                \hline
                \textbf{\small Material Volume} & $[ 2.08 \ ,\ 3.12] \times 10^5 \ (m^3)$ &  $2.6  \times 10^5 \ (m^3)$\\
                                 & {\scriptsize ($\pm20\%$ of mean value)} & \\
                \hline \hline
\end{tabular}
\end{center}
\end{table}
...
\vskip.3cm\noindent \textbf{PF} - $\phi_2 \in [18^{\mathrm{\circ}}, 33^{\mathrm{\circ}}]$, $\quad\Delta\phi_{12} \in [10^{\mathrm{\circ}}, 15^{\mathrm{\circ}}]$, $\quad\beta \in [0.1, 0.85]$, $\quad \mathcal{L}=10^{-1} \ (m)$, $\quad \phi_3=\phi_1 + 1^{\mathrm{\circ}}$.
...
\begin{figure}[H]
        ...
        \caption{Comparison between \emph{Max Flow Height Record} and \emph{Max Velocity Record} maps of simulated flow, assuming Mohr-Coulomb (a),(b), Pouliquen-Forterre (c),(d), and Voellmy-Salm (e),(f) rheology. Extreme cases producing maximum and minimum runout, i.e. (a),(c),(e) \emph{\textbf{max volume -- min resistance}} and (b),(d),(f) \emph{\textbf{min volume -- max resistance}}. Parameters values and input options are also reported.}
...
First case study is the small scale experimental setting described above.

\subsection{Flow Spatial Extents}
...
\begin{figure}[H]
	...
	\begin{minipage}[b]{0.5\linewidth}
    	...
    	\label{fig:Ramp-L4-H}
    \end{minipage}
    \caption{Records of flow height, $h(L,t)$, at four spatial locations of interest. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Different rheology models are displayed separately.}
    \label{fig:Ramp-LM-H}
\end{figure}
\subsubsection*{Flow Height - model comparison on local measurements}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Height/H_mean.png}
        \caption{Mean values of flow height for the three models, $h(L,t)$, recorded at locations of interest, $L_i, \ _{i=1,...,4}$.}
        \label{fig:Ramp-LM-H-means}
\end{figure}

\subsubsection*{Flow Lateral Extent - $E(t)$}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Height/LateralExtent_SpatialRec.png}
        \caption{Comparison between mean values of flow lateral extent as a function of time.}
        \label{fig:Ramp-LatExtension-spatial}
\end{figure}

\subsubsection*{Flow Area - $A(t)$}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Height/Area_SpatialRec.png}
        \caption{Comparison between mean values of flow area as a function of time.}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Records of flow speed, $\Vert \underline{\mathbf{u}} \Vert(L,t)$, at four spatial locations of interest. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Different rheology models are displayed separately.}
    \label{fig:Ramp-LM-Vel}
\end{figure}

\subsubsection*{Flow Speed - model comparison on local measurements}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Velocity/Vel_mean.png}
        \caption{Mean values of flow speed, $\Vert \underline{\mathbf{u}} \Vert(L,t)$, recorded at locations of interest, $L_i, \ _{i=1,...,4}$.}
        \label{fig:Ramp-LM-Vel-means}
\end{figure}

\subsubsection*{Flow Speed - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Velocity/Vel_SpatialRec.png}
        \caption{Spatial average of flow speed. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Records of Froude number at four spatial locations of interest. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Different rheology models are displayed separately.}
    \label{fig:Ramp-LM-Fr}
\end{figure}

\subsubsection*{Froude Number - model comparison on local measurements}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Froude/Fr_mean.png}
        \caption{Mean values of Froude number, $\Vert \underline{\mathbf{u}} \Vert(L,t)$, recorded at locations of interest, $L_i, \ _{i=1,...,4}$.}
        \label{fig:Ramp-LM-Fr-means}
\end{figure}

\subsubsection*{Froude Number - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Froude/Fr_SpatialRec.png}
        \caption{Spatial average of Froude number. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-Fr-spatial}
\end{figure}

\subsection{Flow Acceleration}
\subsubsection*{Flow Acceleration Magnitude - local measurements}
\begin{figure}[H]
	\begin{minipage}[b]{0.5\linewidth}
    	\centering
    ...
    \end{minipage}
    \caption{Records of flow acceleration magnitude (computed from LHS), $\Vert \underline{\mathbf{a}} \Vert(L,t)$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Different rheology models are displayed separately.}
    \label{fig:Ramp-LM-AccL}
\end{figure}

\subsubsection*{Flow Acceleration Magnitude - model comparison on local measurements.}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Acceleration/accel_meanL.png}
        \caption{Mean values of flow acceleration (computed from LHS), $\Vert \underline{a} \Vert(L,t)$, recorded at locations of interest, $L_i, \ _{i=1,...,4}$.}
        \label{fig:Ramp-LM-AccL-means}
\end{figure}


\newpage
\subsection{Forces and Powers}
\subsubsection*{Force LHS$_1$ in the slope direction - spatial average}
\textbf{LHS$_1$ - $\left(\frac{\partial}{\partial t} (h\bar{u}), \frac{\partial}{\partial t} (h\bar{v})\right)$}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/LHS1/FLHS1x.png}
        \caption{Spatial average of force LHS$_1$ in the slope direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS1-Fx-spatial}
\end{figure}
\subsubsection*{Force LHS$_1$ in the lateral direction - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/LHS1/FLHS1y.png}
        \caption{Spatial average of force LHS$_1$ in the lateral direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS1-Fy-spatial}
\end{figure}
\subsubsection*{Power of LHS$_1$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/LHS1/PLHS1.png}
        \caption{Spatial average of the power of LHS$_1$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS1-Power-spatial}
\end{figure}

\newpage
\subsubsection*{Force LHS$_2$ in the slope direction - spatial average}
\textbf{LHS$_2$ - $\left(\frac{\partial}{\partial x}\left(h\bar{u}^2 + \frac{1}{2}k g_{z}h^2\right) + \frac{\partial}{\partial y}(h\bar{u}\bar{v}),\frac{\partial}{\partial x}(h\bar{u}\bar{v}) +
...
\begin{figure}[H]
        ...
        \caption{Spatial average of force LHS$_2$ in the slope direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS2-Fx-spatial}
\end{figure}
\subsubsection*{Force LHS$_2$ in the lateral direction - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/LHS2/FLHS2y.png}
        \caption{Spatial average of force LHS$_2$ in the lateral direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS2-Fy-spatial}
\end{figure}
\subsubsection*{Power of LHS$_2$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/LHS2/PLHS2.png}
        \caption{Spatial average of the power of LHS$_2$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-LHS2-Power-spatial}
\end{figure}

\newpage
\subsubsection*{Force RHS$_1$ in the slope direction - spatial average}
\textbf{RHS$_1$ - $(g_x h,0)$}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS1/FRHS1x.png}
        \caption{Spatial average of force RHS$_1$ in the slope direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS1-Fx-spatial}
\end{figure}
\subsubsection*{Power of RHS$_1$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS1/PRHS1.png}
        \caption{Spatial average of the power of RHS$_1$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS1-Power-spatial}
\end{figure}

\newpage
\subsubsection*{Force RHS$_2$ in the slope direction - spatial average}
\textbf{RHS$_2 = -h g_z\tan(\phi_{bed})\left(\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf u}} \|}, \frac{\bar{v}}{\| \underset{^\sim}{\bar{\textbf u}} \|} \right)$ in Mohr-Coulomb model.}

\noindent\textbf{RHS$_2 = - h g_z \ \mu_{b}(\|\underset{^\sim}{\bar{\textbf{u}}} \| , h)\left(\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf{u}}} \|}, \frac{\bar{v}}{\| \underset{^\sim}{\bar{\textbf{u}}} \|}\right)$ in Pouliquen-Forterre model.}

\noindent\textbf{RHS$_2 = -h g_{z} \mu\left(\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf u}}\|} , \frac{\bar{v}}{\| \underset{^\sim}{\bar{\textbf u}}\|}\right)$ in Voellmy-Salm model.}
...
\end{figure}
\subsubsection*{Force RHS$_2$ in the lateral direction - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS2/FRHS2y.png}
        \caption{Spatial average of force RHS$_2$ in the lateral direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS2-Fy-spatial}
\end{figure}
\subsubsection*{Power of RHS$_2$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS2/PRHS2.png}
        \caption{Spatial average of the power of RHS$_2$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS2-Power-spatial}
\end{figure}

\newpage
\subsubsection*{Force RHS$_3$ in the slope direction - spatial average}
\textbf{RHS$_3$ - $ \left(-\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf u}} \|} h \frac{\bar{u}^2}{r_x} \tan(\phi_{bed}),0\right)$ in Mohr-Coulomb model.}

\noindent\textbf{RHS$_3$ - $\left(-\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf{u}}} \|} h \frac{\bar{u}^2}{r_x} \ \mu_{b}\|\underset{^\sim}{\bar{\textbf{u}}} \| , 0\right)$ in Pouliquen-Forterre model.}

\noindent\textbf{RHS$_3$ - $\left(- \frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf u}}\|} \ h \frac{\| \underset{^\sim}{\bar{\textbf u}} \|^2}{r_{x}} \mu,0\right)$ in Voellmy-Salm model.}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS3/FRHS3x.png}
        \caption{Spatial average of force RHS$_3$ in the slope direction. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS3-Fx-spatial}
\end{figure}
\subsubsection*{Power of RHS$_3$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS3/PRHS3.png}
        \caption{Spatial average of the power of RHS$_3$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS3-Power-spatial}
\end{figure}

\newpage
\subsubsection*{Force RHS$_4$ in the slope direction - spatial average}
\textbf{RHS$_4 = - h k_{ap}\sin(\phi_{int})\left( \ {\rm sgn}(\frac{\partial \bar{u}}{\partial y}) \frac{\partial (g_z h)}{\partial y},\ {\rm sgn}({\frac{\partial \bar{v}}{\partial x}}) \frac{\partial (g_z h)}{\partial x}\right)$ in Mohr-Coulomb model.}

\noindent\textbf{RHS$_4 = g_{z}h\left(\frac{\partial h}{\partial x}, \frac{\partial h}{\partial y}\right)$ in Pouliquen-Forterre model.}

\noindent\textbf{RHS$_4 = -\frac{\| \underset{^\sim}{\textbf g} \|}{\xi}\| \underset{^\sim}{\bar{\textbf u}} \|^2\left(\frac{\bar{u}}{\| \underset{^\sim}{\bar{\textbf u}}\|} \ ,\frac{\bar{v}}{\| \underset{^\sim}{\bar{\textbf u}}\|}\right)$ in Voellmy-Salm model.}
...
\end{figure}
\subsubsection*{Force RHS$_4$ in the lateral direction - spatial average}
\begin{figure}[H]
        \centering
...
\end{figure}
\subsubsection*{Power of RHS$_4$ - spatial average}
\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Forces_Powers/RHS4/PRHS4.png}
        \caption{Spatial average of the power of RHS$_4$. Bold line is mean value, dashed lines are 5$^th$ and 95$^th$ percentile bounds. Model comparison on the mean value is also displayed.}
        \label{fig:Ramp-RHS4-Power-spatial}
\end{figure}


\subsection{Dominance factors - RHS terms}

\begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{InclinedPlane/Dominance1/DominanceCX.png}
        \caption{Probability of events $(A_i)_{i=1,...,4}:=\left\{\Phi_2=|RHS_i|\right\}$ for the \emph{Mohr-Coulomb} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the slope direction.}
...
\begin{figure}[H]
        ...
        \caption{Probability of events $(A_i)_{i=1,...,4}:=\left\{\Phi_2=|RHS_i|\right\}$ for the \emph{Mohr-Coulomb} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the lateral direction.}
...
\begin{figure}[H]
        ...
        \caption{Probability of events $(A_i)_{i=1,...,6}:=\left\{\Phi_2=|RHS_i|\right\}$ for the \emph{Pouliquen-Forterre} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the slope direction.}
...
\begin{figure}[H]
        ...
        \caption{Probability of events $(A_i)_{i=1,...,6}:=\left\{\Phi_2=|RHS_i|\right\}$ for the \emph{Pouliquen-Forterre} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the lateral direction.}
...
\begin{figure}[H]
        ...
        \caption{Probability of events $(A_i)_{i=1,...,6}:=\left\{\Phi_2=|RHS_i|\right\}$ for \emph{Voellmy-Salm} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the slope direction.}
...
\begin{figure}[H]
        ...
        \caption{Probability of events $(A_i)_{i=1,...,6}:=\left\{\Phi_2=|RHS_i|\right\}$ for \emph{Voellmy-Salm} model. Moreover, $A_7$ correspond to the event of \emph{no flow}. All the measurements were recorded along the lateral direction.}
        \label{fig:Ramp-FDominance1-VY}
\end{figure}

\subsection{Contribution coefficients - LHS and RHS terms}

\section{QoIs and Data Collected - Volc{\'a}n de Colima block and ash flow}\label{QoI2}
