...
Our statistical analysis based on UQ depicts three different models, due to the different assumptions underlying to them. Compared to a classical MC model, PF lacks of internal friction and this produced an accentuated lateral spread. This is increased by the hydrostatic correction, which pushes the flow ahead and laterally during the initial collapse. This can have some minor additional effects in the the deposit. The interpolation of the bed friction angle $\phi_1$ to a larger value $\phi_2$ where the flow thins compared to its speed, stops the flow more suddenly, and does not allow large velocity peaks. Instead in VS, the speed dependent friction has a great effect in reducing lateral spread and producing channeling features even due to minor ridges in topography. The flow tends to be significantly slower and more distally stretched in the slope direction. The potentially higher curvature term formulation is not enough to balance the effect of the lower basal friction and speed, and hence is not significantly affecting the results.
